class GetAllUnitsDetailsModel {
  bool? status;
  UnitsOfData? units;

  GetAllUnitsDetailsModel({this.status, this.units});
...
class UnitsOfData {
  List<AllUnitsData>? data;
  Links? links;
  Meta? meta;

  UnitsOfData({this.data, this.links, this.meta});

  UnitsOfData.fromJson(Map<String, dynamic> json) {
    if (json['data'] != null) {
      data = <AllUnitsData>[];
      json['data'].forEach((v) {
        data!.add(new AllUnitsData.fromJson(v));
      });
    }
